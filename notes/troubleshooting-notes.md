# Troubleshooting Notes — Windows Script Host Abuse
...
### 1. Process Name Alone Is Not Enough

`wscript.exe` and `cscript.exe` are legitimate Windows components.

Their presence alone does not establish malicious activity.
...
finds child processes launched by that parent.

### 3. Known Artifacts Improve Hunting

A known script name or path can be more effective than a broad process-name query when validating controlled activity.

### 4. Telemetry Anomalies Need Validation

Unexpected metadata should be recorded and correlated with other fields instead of being treated as an immediate security finding.

### 5. Evidence Must Drive the Assessment

The lab intentionally generated benign activity.

The investigation therefore distinguishes:
